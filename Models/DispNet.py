...
class DispNet(nn.Module):
    ...
    def __init__(self, batchNorm=True):
        ...
        self.conv2 = conv(self.batchNorm, 32, 128, kernel_size=5, stride=2)
        ...
        self.iconv1 = nn.Conv2d(65, 32, kernel_size=3, stride=1, padding=1, bias=True)
    ...
    def forward(self, x):
        ...
        pr5 = self.predict_flow5(out_conv5_b)
        pr5_up = self.upsampled_flow5_to_4(pr5)

        upconv4 = self.upconv4(out_conv5_b)
...
''' endpoint error '''
def lossEPE(output_list, weights, target):
    b, _, h, w = target.size()
    loss = []
    meanloss = 0
    for i in range(len(output_list)):
      upsampled_output = F.interpolate(output_list[i], (h, w), mode='bilinear', align_corners=False)
      loss.append(torch.norm(target - upsampled_output, dim=1, p=2).mean() * weights[i])
      meanloss += torch.norm(target - upsampled_output, dim=1, p=2).mean() * weights[i]
    loss.append(meanloss)
    return loss, meanloss
